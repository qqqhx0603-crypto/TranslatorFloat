...
- 支持临时提示词：勾选后，只有输入严格以小写 `oder:` 开头时才解析；`oder:` 到第一个中文句号 `。` 之间的内容只作为本次翻译额外要求，可控制目标语、术语、风格、大小写、固定输出、格式等本次输出约束；无法作用于译文的聊天/闲聊内容会被忽略；常见硬约束如“翻译结果输出 1”“全大写/全小写”会由程序兜底，并且不会再发送给模型

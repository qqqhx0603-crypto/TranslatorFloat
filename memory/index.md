...
- 维护两个完全分离的版本：`python` Python 自包含版、`powershell` PowerShell 自包含版。

## 重要约束

- 不泄露 API Key。
- 不随意删除或重置 `translation_cache.json`、`window_settings.json`。
- Python 版本相关文件必须留在 `python` 文件夹内，PowerShell 版本相关文件必须留在 `powershell` 文件夹内，便于任一文件夹直接压缩分发。
- 所有项目操作和产物必须限制在当前项目根目录内；目录变化后先重新确认实际文件树，不沿用旧路径。

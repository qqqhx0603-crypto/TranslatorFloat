...
- 支持自定义快速/思考翻译提示词。
- 支持临时提示词：开启后，只有输入严格以小写 `oder:` 开头时才解析；`oder:` 到第一个中文句号 `。` 之间的内容只作为本次翻译额外要求，不作为原文翻译，可控制目标语、术语、风格、大小写、固定输出、格式等本次输出约束；无法作用于译文的聊天/闲聊内容会被忽略，不能被输出或回应。常见硬约束如“翻译结果输出 1”“全大写/全小写”会由程序兜底处理，降低不同模型不听提示词的影响。
- 支持翻译缓存。
